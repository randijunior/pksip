...
use crate::{
    ...
    find_map_header,
    message::{
        DomainName, Host, HostPort, MandatoryHeaders, NameAddr, ReasonPhrase, Request, Response,
        Scheme, SipMessage, SipMessageBody, SipUri, StatusCode, StatusLine, Uri, UriBuilder,
        headers::{
            CSeq, CallId, ContentLength, From as FromHeader, Header, Headers, MaxForwards, To, Via,
        },
    },
    parser::{HeaderParser, SipMessageParser},
    transaction::{ClientTransaction, ServerTransaction},
...
impl TransportMessage {
    ...
    pub fn parse(&self) -> Result<SipMessage> {
        ...
        let sip_message = match SipMessageParser::parse(&packet.data) {
            Ok(parsed) => parsed,
            Err(err) => {
                log::warn!(
