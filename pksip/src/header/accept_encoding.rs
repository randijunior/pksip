...
///
/// ```
/// # use pksip::{header::{AcceptEncoding, Coding}};
...
impl<'a> HeaderParser<'a> for AcceptEncoding {
...
mod tests {
    ...
    fn test_parse_simple_codings() {
        ...
        assert_eq!(coding.coding.as_ref(), "compress");
        assert_eq!(coding.q, None);

        let coding = accept_encoding.get(1).unwrap();
        assert_eq!(coding.coding.as_ref(), "gzip");
        assert_eq!(coding.q, None);
    }
    ...
    fn test_parse_wildcard_coding() {
        ...
        let coding = accept_encoding.get(0).unwrap();
        assert_eq!(coding.coding.as_ref(), "*");
        assert_eq!(coding.q, None);
    }
    ...
    fn test_parse_with_q_params() {
        ...
        let coding = accept_encoding.get(1).unwrap();
        assert_eq!(coding.coding.as_ref(), "identity");
