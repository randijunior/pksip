...
///
/// ```
/// # use pksip::{header::{AcceptLanguage, Language}};
...
impl<'a> HeaderParser<'a> for AcceptLanguage {
     ...
     * accept-param) language-range   =  ( ( 1*8ALPHA *(
     * "-" 1*8ALPHA ) ) / "*" )
     */
...
mod tests {
    ...
    fn test_parse() {
        ...
        assert_eq!(lang.language.as_ref(), "en");
        ...
        let lang = accept_language.get(0).unwrap();
        assert_eq!(lang.language.as_ref(), "da");
        assert_eq!(lang.q, None);

        let lang = accept_language.get(1).unwrap();
        assert_eq!(lang.language.as_ref(), "en-gb");
        ...
        let lang = accept_language.get(0).unwrap();
        assert_eq!(lang.language.as_ref(), "*");
        assert_eq!(lang.q, None);
    }
}
