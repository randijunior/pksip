...
impl<'a> HeaderParser<'a> for ContentType {
    const NAME: &'static str = "Content-Type";
    const SHORT_NAME: &'static str = "c";

    /*
     * Content-Type     =  ( "Content-Type" / "c" ) HCOLON
     * media-type media-type       =  m-type SLASH
     * m-subtype *(SEMI m-parameter) m-type           =
     * discrete-type / composite-type discrete-type    =
     * "text" / "image" / "audio" / "video"
     * / "application" / extension-token composite-type
...
mod tests {
    ...
    fn test_parse() {
        ...
        assert_eq!(c_type.0.mimetype.mtype.as_ref(), "application");
        assert_eq!(c_type.0.mimetype.subtype.as_ref(), "sdp");
        ...
        assert_eq!(scanner.remaining(), b"\r\n");
        assert_eq!(c_type.0.mimetype.mtype.as_ref(), "text");
        assert_eq!(c_type.0.mimetype.subtype.as_ref(), "html");
        assert_eq!(
