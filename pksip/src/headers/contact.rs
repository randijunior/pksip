...
use crate::{
    Q,
    error::Result,
    headers::{EXPIRES_PARAM, HeaderParser, Q_PARAM},
...
impl<'a> HeaderParser<'a> for Contact {
    const NAME: &'static str = "Contact";
    const SHORT_NAME: &'static str = "m";

    /*
     * Contact        =  ("Contact" / "m" ) HCOLON
     *                   ( STAR / (contact-param *(COMMA
     * contact-param))) contact-param  =  (name-addr /
     * addr-spec) *(SEMI contact-params) name-addr      =
     * [ display-name ] LAQUOT addr-spec RAQUOT addr-spec
     * =  SIP-URI / SIPS-URI / absoluteURI display-name
