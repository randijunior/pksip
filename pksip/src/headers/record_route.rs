...
impl<'a> HeaderParser<'a> for RecordRoute {
    const NAME: &'static str = "Record-Route";

    /*
     * Record-Route  =  "Record-Route" HCOLON rec-route
     * *(COMMA rec-route) rec-route     =  name-addr *(
     * SEMI rr-param ) rr-param      =  generic-param
     */
