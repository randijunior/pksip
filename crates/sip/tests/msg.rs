...
use sip::{
    headers::Header,
    msg::{Host, Scheme, SipMethod, TransportProtocol},
...
fn test_parse_msg_1() {
    ...
    assert_matches!(
        ...
        Some(Header::Via(via)) => {
            ...
            assert_eq!(via.sent_by.host, Host::DomainName("pc33.atlanta.com"));
    ...
    assert_matches!(iter.next(), Some(Header::To(to)) => {
        ...
        assert_eq!(name_addr.display, Some("Bob"));
        assert_eq!(name_addr.uri.host_port.host, Host::DomainName("biloxi.com"));
    ...
    assert_matches!(iter.next(), Some(Header::From(from)) => {
        ...
        assert_eq!(name_addr.display, Some("Alice"));
        assert_eq!(name_addr.uri.host_port.host, Host::DomainName("atlanta.com"));
        assert_eq!(name_addr.uri.scheme, Scheme::Sip);
        assert_eq!(from.tag, Some("1928301774"));
    });
    assert_matches!(iter.next(), Some(Header::CallId(c)) => {
        assert_eq!(c.0, "a84b4c76e66710@pc33.atlanta.com");
    });
    ...
    assert_matches!(iter.next(), Some(Header::Contact(c)) => {
        let uri = &c.uri().unwrap().uri.name_addr().unwrap().uri;
        assert_eq!(uri
            .host_port
            .host, Host::DomainName("pc33.atlanta.com"));
        assert_eq!(uri.user.as_ref().unwrap().get_user(), "alice");
    });
    assert_matches!(iter.next(), Some(Header::ContentType(c)) => {
        assert_eq!(c.0.mimetype.mtype, "application");
        assert_eq!(c.0.mimetype.subtype, "sdp");
    });
...
fn test_parse_msg_2() {
    ...
    assert_matches!(
        ...
        Some(Header::Via(via)) => {
            assert_eq!(via.transport, TransportProtocol::UDP);
            assert_eq!(via.sent_by.host, Host::DomainName("bobspc.biloxi.com"));
    ...
    assert_matches!(iter.next(), Some(Header::CallId(c)) => {
        assert_eq!(c.0, "843817637684230@998sdasdh09");
    });
...
#[test]
fn test_parse_msg3() {
    let msg = "REGISTER sip:localhost:8080;transport=UDP SIP/2.0\r\nVia: SIP/2.0/UDP 127.0.0.1:49681;branch=z9hG4bK-524287-1---5920ecc586013465;rport\r\nMax-Forwards: 70\r\nContact: <sip:100010@127.0.0.1:49681;rinstance=b9c5991b65d6c4c3;transport=UDP>\r\nTo: <sip:100010@localhost:8080;transport=UDP>\r\nFrom: <sip:100010@localhost:8080;transport=UDP>;tag=85e09d56\r\nCall-ID: aSOl06sRYzct1Vrk2KoKOA..\r\nCSeq: 1 REGISTER\r\nExpires: 60\r\nAllow: INVITE, ACK, CANCEL, BYE, NOTIFY, REFER, MESSAGE, OPTIONS, INFO, SUBSCRIBE\r\nSupported: replaces, norefersub, extended-refer, timer, sec-agree, outbound, path, X-cisco-serviceuri\r\nUser-Agent: Z 5.6.1 v2.10.19.9\r\nAllow-Events: presence, kpml, talk, as-feature-event\r\nContent-Length: 0\r\n\r\n";

    let msg = parse_sip_msg(msg.as_bytes()).unwrap();
}
